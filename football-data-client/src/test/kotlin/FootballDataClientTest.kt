...
class FootballDataClientTest {
    ...
    fun serieA_MatchEnded() {
        ...
        val url = server.url("/")
        val client = FootballDataClient.create(url.toString(), "some-auth-token");
        val call = client.getFixturesByLeagueAndRound(FootballDataClient.SERIE_A_LEAGUE_ID, "Regular_Season_-_23")
        val fixtures: List<Fixture>? = call.execute().body()?.api?.fixtures
